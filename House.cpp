...
void House::draw(DrawData& data)
{
	...
	for (int i = 0; i < 60; i++){
		...
		//printf("%f, %f, %f indices = %d i = %d \n", vertices[indices[i] * 3], vertices[indices[i] * 3 + 1], vertices[indices[i] * 3 + 2], indices[i], i);
		//printf("%f, %f, %f \n", colors[indices[i] * 3], colors[indices[i] * 3 + 1], colors[indices[i] * 3 + 2]);
	}
